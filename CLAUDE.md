...
- `POST /api/v1/file/disable` — body `{"file": NAME}`; disables a config file by renaming it to `NAME.disabled` via `os.rename` — `json2nix.nix`'s `walk()` only picks up names ending in exactly `.json`, so a `.json.disabled` file drops out of the Nix merge with no change needed there, the same trick `custom-options.json` already uses; rejects if `NAME` isn't a `.json` file or a disabled sibling already exists (auth required)
- `POST /api/v1/file/enable` — body `{"file": NAME}` where `NAME` ends in `.json.disabled`; reverses `file/disable`, stripping the `.disabled` suffix; rejects if the destination `.json` already exists (auth required)
- `POST /api/v1/folder/disable` — body `{"folder": NAME}`; disables a whole subfolder (and everything nested inside it) by renaming it to `.NAME.disabled` — dot-prefixed, so `json2nix.nix`'s `walk()` skips it via the same dotdir check that already excludes `.ezconf-backups`, no changes to `json2nix.nix` needed; rejects if `NAME` is already a disabled folder or a disabled sibling already exists (auth required)
