...
- `POST /api/v1/file/delete` — body `{"file": NAME}`; deletes a whole config file; does not itself create a backup (that state is only saved if/when the user later hits Save on another tab); refuses to delete the last remaining file (auth required)
