    ...
    buttons = lib.mkOption {
      type = lib.types.listOf (lib.types.submodule {
        options = {
          ...
          direct      = lib.mkOption { type = lib.types.bool; default = false; description = "Run this command as an independent OS process instead of typing it into the terminal shell. Meant for a command that might restart ezconf's own services mid-run (nixos-rebuild switch being the motivating case) -- ordinary buttons run inside ezconf-terminal.service, so a command that restarts that service kills the very shell it's running in; a direct command runs detached from any of ezconf's own systemd units, survives either service restarting, and streams its output live into its own popup panel instead of the terminal panel. Requires systemd (Linux only)."; };
        };
      });
      default     = [];
