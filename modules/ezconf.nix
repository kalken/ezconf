    ...
    buttons = lib.mkOption {
      type = lib.types.listOf (lib.types.submodule {
        options = {
          ...
          terminal    = lib.mkOption { type = lib.types.bool; default = false; description = "Type this command into the terminal shell instead of running it as an independent OS process (the default for every button). Set this for a command that specifically needs the terminal -- an interactive prompt, wanting to see it run inline alongside other terminal work, or clear_first. Do NOT set this for a command that might restart ezconf's own services mid-run (nixos-rebuild switch being the motivating case): it runs inside ezconf-terminal.service, so a command that restarts that service would kill the very shell it's running in. The default (independent process) runs detached from any of ezconf's own systemd units, survives either service restarting, and streams its output live into its own popup panel instead of the terminal panel. That default requires systemd (Linux only); terminal = true has no such requirement, since it just types into whatever shell the terminal panel already has."; };
        };
      });
      default     = [];
