...
- Browse any `.md` files in the system root (e.g. `README.md`) in a side panel
...
| `terminalPersist` | bool | `false` | Run the terminal's shell in a persistent `dtach` session, so a running command survives the terminal service restarting or a browser reconnect (requires `dtach`; see below) |
...
- With `terminalPersist = false` (the default), the terminal service has `restartIfChanged = false` — active terminal sessions survive `nixos-rebuild switch`, but restarting it yourself (e.g. `systemctl restart ezconf-terminal` to pick up a package update) still kills whatever's running inside it. Set `terminalPersist = true` to survive that too: the shell moves into its own persistent session (`ezconf-terminal-session.service`, via `dtach`), and reconnecting (or the terminal service itself restarting — which now happens automatically again when its package/config changes, since doing so no longer loses anything) just re-attaches to it instead of starting fresh. That holds even if a rebuild is what's running *in* the terminal panel when the bridge restarts — the rebuild lives in the separate session, untouched either way. This doesn't survive an actual reboot — nothing can, the process itself is gone — only restarts of the services around it, within the same boot. Requires `dtach` to be installed.
