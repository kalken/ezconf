...
`mode = "internal"` moves a button into the terminal panel's own header bar instead, next to the size/show-hide controls, rather than the command row — a spot that's always visible regardless of `services.ezconf.mode`. Whenever `terminal = true`, ezconf adds one such button automatically: **Restart Terminal**, which runs `systemctl restart ezconf-terminal.service`. Every currently running shell — across every connected browser — ends the moment it runs, so only use it when you actually mean to reset a stuck or misbehaving terminal session; reopening the panel afterward reattaches to a fresh shell as normal, the same as after a reboot.

A small dot appears on that button whenever the running terminal service is out of date with what a rebuild actually installed — e.g. you upgraded ezconf but the terminal panel hasn't been restarted since. It's just a passive notice, not automatic: restarting kills every connected shell, so it stays something you trigger yourself, whenever you're ready.
...
| `terminalPersist` | bool | `false` | Keep the one shared terminal shell running server-side across a dropped/closed connection (browser closed, network drop, logout), so reconnecting reattaches instead of starting fresh |
