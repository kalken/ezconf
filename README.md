...
| `terminalPersist` | bool | `false` | Run the terminal's shell in a persistent `tmux` session, so a running command survives the terminal service restarting or a browser reconnect (requires `tmux`; see below) |
...
- With `terminalPersist = false` (the default), the terminal service has `restartIfChanged = false` — active terminal sessions survive `nixos-rebuild switch`, but restarting it yourself (e.g. `systemctl restart ezconf-terminal` to pick up a package update) still kills whatever's running inside it. Set `terminalPersist = true` to survive that too: the shell moves into its own persistent session (`ezconf-terminal-session.service`, via `tmux`), and reconnecting (or the terminal service itself restarting — which now happens automatically again when its package/config changes, since doing so no longer loses anything) just re-attaches to it instead of starting fresh; anything that happened while disconnected is still there in the session's own tmux scrollback (Shift+PageUp). That holds even if a rebuild is what's running *in* the terminal panel when the bridge restarts — the rebuild lives in the separate session, untouched either way. This doesn't survive an actual reboot — nothing can, the process itself is gone — only restarts of the services around it, within the same boot. Requires `tmux` to be installed.
- **Changing `terminalPersist` itself must be applied from outside the ezconf terminal panel** — SSH, physical/serial console, whatever's available. Flipping it either direction tears down whichever architecture is currently hosting the connection running the switch (restarting `ezconf-terminal.service` going false→true, or removing `ezconf-terminal-session.service` entirely going true→false), killing that `nixos-rebuild switch` command mid-flight if you run it from inside the panel. Any rebuild that *doesn't* touch this setting is unaffected and safe to run from the terminal panel as normal.
