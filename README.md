...
- The terminal service has `restartIfChanged = false` — active terminal sessions survive `nixos-rebuild switch`, but restarting it yourself (e.g. `systemctl restart ezconf-terminal` to pick up a package update) still kills whatever's running inside it, since the shell is forked directly into that connection's own PTY.
